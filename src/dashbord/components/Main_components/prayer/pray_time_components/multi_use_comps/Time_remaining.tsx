...
const [timeRemaining, setTR] = useState("00:00");
  ...
  const mosqueIcama = prayer_time ? icama[prayer_time.title as keyof typeof icama] : undefined;
  ...
  useEffect(() => {
    ...
    const intervalId = setInterval(() => {
      ...
      if (remaining_hour === 0 && remaining_mins === 0 && f(second) === 0) {
        navigate(`/adhan/${prayer_time?.title}/${mosqueIcama ?? 5}/${id}`);
      }

      if (remaining_hour >= 0 && remaining_mins >= 10 &&  f(second) === 0 &&date.getMinutes() % 5 === 0) {
        navigate(`/adkar/${id}`);
      }
    }, 1000);
