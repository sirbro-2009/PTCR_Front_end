  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-4 self p-4    w-full">
      {(data.searchArrayRes).map(
          ...
          return (
            <Card key={i} className="flex flex-col  m-2" dir={document.dir}>
              ...
              <div className="flex justify-between  flex-row-reverse items-center">
                ...
                style={{ color: metadata?.color }} className="flex  flex-row-reverse m-2 bg-popover-foreground/10 rounded-lg p-1 text-2xl items-center">
                  <span className="text-xl">{keyValue.length >= 10?keyValue.length:"0"+keyValue.length}</span>
                  
                  <TiDocumentText />
                </div>
              </div>
              {/**adkar type */}
              <Label className=" text-center m-auto">{e.replaceAll("_"," ")}</Label>
              <Button className="w-1/2 rounded m-auto cursor-pointer" 
              variant='secondary'
              onClick={()=>{
        ...
        return (<Card className="m-auto w-full md:w-fit p-2">
            <Label className="m-auto text-2xl">:( No result found </Label>
